...
export default function ArkyUnverfiiedCollectionsPage({ ArkyCollections, Loading }) {
    let projectCount = 1
    let getUnverifiedCollections = ArkyCollections.map((data) => {
        ...
        return (
            <>
                {data.isScam === false && data.isVerified === false && CollectionName.includes("To Remove") === false ? <tbody className="bg-gray-100" key={data.tokenAddress}><tr className="text-gray-700">
                        ...
                        <div className="flex items-center text-sm">
                            ...
                            <div>
                                <p className="font-semibold text-black underline hover:text-[#299197]">
                                    <Link href={`/arky/unverifiedcollections/${encodeURIComponent(data.collectionsName)}`}>{data.collectionsName}</Link>
                                </p>
                            </div>
                        </div>
                    </td>
